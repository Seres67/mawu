...
use crate::lexer::{SyntaxKind, Lexeme};
use crate::syntax::MawuLanguage;
...
impl<'l, 'input> Sink<'l, 'input> {
        ...
        for event in reordered_events {
            match event {
                Event::StartNode { kind } => {
                    self.builder.start_node(MawuLanguage::kind_to_raw(kind))
                }
    ...
    fn token(&mut self, kind: SyntaxKind, text: SmolStr) {
        self.builder.token(MawuLanguage::kind_to_raw(kind), text);
        self.cursor += 1;
    }
